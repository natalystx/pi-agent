...
// Plugin keys (e.g. "pstack@pstack-claude") to keep out of pi even though they are
// enabled in Claude Code. Edit ~/.pi/agent/claude-plugins-ignore.json, then /reload.
const IGNORE_LIST_PATH = path.join(os.homedir(), ".pi", "agent", "claude-plugins-ignore.json");
...
async function loadIgnoredPluginKeys(): Promise<Set<string>> {
  let raw: string;
  try {
    raw = await readFile(IGNORE_LIST_PATH, "utf8");
  } catch (error) {
    const code = (error as NodeJS.ErrnoException).code;
    if (code === "ENOENT") return new Set();
    throw error;
  }
  const parsed = JSON.parse(raw) as { ignore?: string[] };
  return new Set(parsed.ignore ?? []);
}
...
async function loadEnabledPluginKeys(cwd: string): Promise<Set<string>> {
  ...
  const ignoredPluginKeys = await loadIgnoredPluginKeys();
  const normalizedCwd = normalizePath(cwd);
  const enabled = new Set<string>();

  for (const [pluginKey, entries] of Object.entries(plugins)) {
    if (pluginEnabledStates[pluginKey] === false || ignoredPluginKeys.has(pluginKey)) {
      continue;
    }
